...
pub fn process_cpu_instructions<'a>(input: &Vec<&'a str>) -> Vec<i32> {
    let mut x: i32 = 1;
...
pub fn part_one(input: &Vec<&str>) -> i32 {
    let lengths = process_cpu_instructions(input);
...
```rust
pub fn compute_new_sprite_position(x: &i32) -> Range<i32> {
    let start = x - 1;
    let end = x + 2;
...
pub fn parse_cpu_instructions(input: &Vec<&str>) -> Vec<CPUInstruction> {
    ...
    let mut next_available_cycle = 1;
    ...
    for line in input {
        match line {
            ...
            _ => panic!("Invalid CPU instruction"),
        }
    }
...
pub fn process_cpu_instructions<'a>(input: &Vec<&'a str>) -> (Vec<i32>, Vec<Vec<&'a str>>) {
    let mut crt_screen = create_crt_screen();
    let mut sprite_position = 0..3;
    let mut crt_row_idx = 0;
    let mut x: i32 = 1;
    ...
    let last_scheduled_cmd = pending_work.last().unwrap().finishes_at;
    (1..=last_scheduled_cmd).for_each(|cycle| {
        crt_row_idx = (cycle - 1) / 40;
        let crt_row = crt_screen.get_mut(crt_row_idx).unwrap();
        let row_idx = (cycle as i32 - 1).rem_euclid(40) as usize;

        crt_row[row_idx] = match &sprite_position {
            p if p.contains(&(row_idx as i32)) => "#",
            _ => ".",
        };
        ...
        sprite_position = compute_new_sprite_position(&x);
    });

    (signal_lengths, crt_screen)
}

pub fn part_two<'a>(input: &Vec<&'a str>) -> Vec<Vec<&'a str>> {
